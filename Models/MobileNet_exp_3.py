...
class ConvNet_w_MN_3(torch.nn.Module):
    def __init__(self):
        super(ConvNet_w_MN_3, self).__init__()
        mobile_net = mobilenet_v3_small(pretrained=True)
        self.features = mobile_net.features
        self.avgpool = mobile_net.avgpool
        self.linear1 = torch.nn.Linear(576, 8192)
        self.linear_localize = torch.nn.Linear(8192, 4)
        self.linear_classify = torch.nn.Linear(8192, 1)
